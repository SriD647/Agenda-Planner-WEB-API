...
Artillery was chosen for load testing to assess how well the application can handle its expected workload and to help determine the system's throughput. The test includes both POST and GET methods for a data entity. My test results were all positive, as all requests returned a status code of 200. In addition, the speed of the response time was relatively fast. To run artillery, run `npm run load` in terminal, being sure to be in the right folder.<br>
